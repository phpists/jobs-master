            ...
            <div class="row">
                <form action="{{ route('blogs.update',$blog->id) }}" enctype="multipart/form-data" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-12">
                            <textarea class="summernote" name="description">{!! old('description') ? old('description') : $blog->description !!}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-12">
                            <select class="chosen-select" id="subcategoriesList" data-old="{{ old('subcategory_id') ? old('subcategory_id') : $blog->subcategory_id }}" name="subcategory_id">
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-12">
                            <select multiple="multiple" data-old="{{ json_encode(old('role_ids')) }}"
                                ...
                                @foreach(\App\Role::all() as $route)
                                    ...
                                    @else
                                        @if(!empty($blog->roles()->get()->pluck('id')->toArray()) && in_array($route->id,$blog->roles()->get()->pluck('id')->toArray()))
                                            <option selected value="{{ $route->id }}">{{ $route->name  }}</option>
                                        @else
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-12">
                            <input type="text" name="date" value="{{ old('date') ? old('date') : $blog->date }}" class="datepicker last_date_for_registration" data-date-format="yyyy-mm-dd">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-12">
                            ...
                            @if($blog->image)
                                <img src="{{ '/storage/blogs/'.$blog->image }}" height="200">
                            @endif
                        </div>
                    </div>
                    <div class="form-group">
